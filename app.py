...
# Função para plotar gráficos na escala original
...
class ModeloSES(ModeloPrevisao):
    ...
    def prever(self, mostrar_previsao_futura=False, escolha_periodo=0):
        ...
        plotar_grafico(self.df_treino, self.df_valid, self.df_valid['previsoes_v1'], 'Simple Exponential Smoothing Forecasting', previsoes_futuras)
...
class ModeloDES(ModeloPrevisao):
    ...
    def prever(self, mostrar_previsao_futura=False, escolha_periodo=0):
        ...
        if mostrar_previsao_futura:
            previsoes_futuras = self.modelo.forecast(escolha_periodo)
            previsoes_futuras = pd.Series(previsoes_futuras, index=pd.date_range(start=self.df_valid.index[-1], periods=escolha_periodo, freq='M'))
        plotar_grafico(self.df_treino, self.df_valid, self.df_valid['previsoes_v2'], 'Double Exponential Smoothing Forecasting', previsoes_futuras)
...
class ModeloTES(ModeloPrevisao):
    ...
    def prever(self, mostrar_previsao_futura=False, escolha_periodo=0):
        ...
        if mostrar_previsao_futura:
            previsoes_futuras = self.modelo.forecast(escolha_periodo)
            previsoes_futuras = pd.Series(previsoes_futuras, index=pd.date_range(start=self.df_valid.index[-1], periods=escolha_periodo, freq='M'))
        plotar_grafico(self.df_treino, self.df_valid, self.df_valid['previsoes_v3'], 'Triple Exponential Smoothing Forecasting', previsoes_futuras)
